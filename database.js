...
{
    const[rows] = await pool.query("select * from notes1");
    return rows;
}

async function getNote(id)
{
    const[rows] = await pool.query(`select * from notes1 where id=?`,[id]);
    return rows[0];
}
async function createNote(title,contents)
{
    const[result] = await pool.query(`insert into notes1(title,contents) values(?,?)`,[title,contents]);
    const id=result.insertId;
    return getNote(id);
}

async function deleteNote(id) {
    const [result] = await pool.query('DELETE FROM notes1 WHERE id = ?', [id]);
  ...
  async function updateNote(id, newTitle, newContents) {
    const [result] = await pool.query('UPDATE notes1 SET title = ?, contents = ? WHERE id = ?', [newTitle, newContents, id]);
